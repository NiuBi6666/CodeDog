...
public class RankingService {
      ...
      for (RankingPayload.LessonData lesson : safe(clazz.lessons())) {
        ...
        for (RankingPayload.StudentResult student : safe(lesson.students())) {
          try {
            ...
            if (hash.equals(existingHash(campId, classId, lessonId, studentId))) unchanged++;
            else { upsertResult(campId, classId, lessonId, studentId, score, hash, batchId); changed++; }
          } catch (RuntimeException error) {
  ...
  public RankingPayload.Board board(String campValue, String classValue, String scopeValue) {
    ...
    String sql = """
      ...
      COUNT(r.lesson_id) lesson_count FROM ranking_students s
      JOIN ranking_classes c ON c.camp_id=s.camp_id AND c.class_id=s.class_id
      LEFT JOIN ranking_lesson_results r ON r.camp_id=s.camp_id AND r.class_id=s.class_id AND r.student_id=s.student_id
      WHERE s.camp_id=?
      """ + filter + " GROUP BY s.student_id ORDER BY total_points DESC,homework_points DESC,inclass_points DESC,completion_points DESC,s.student_id";
    List<RankingPayload.Entry> entries = new ArrayList<>();
    jdbc.query(sql, args.toArray(), rs -> {
      int total = rs.getInt("total_points"), level = level(total);
      entries.add(new RankingPayload.Entry(entries.size()+1, rs.getString("student_id"), rs.getString("student_name"),
        rs.getString("class_id"), rs.getString("class_name"), total, rs.getInt("completion_points"),
        rs.getInt("inclass_points"), rs.getInt("homework_points"), rs.getInt("lesson_count"), level, levelName(level)));
    });
    return new RankingPayload.Board(campId, campName, classId, className, scope, entries.size(), latestUpdate(), List.copyOf(entries));
  }
  ...
  private String existingHash(String camp,String clazz,String lesson,String student){return lookup("SELECT content_hash FROM ranking_lesson_results WHERE camp_id=? AND class_id=? AND lesson_id=? AND student_id=?",camp,clazz,lesson,student);}
