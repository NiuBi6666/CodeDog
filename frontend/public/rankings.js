...
function trendBadge(row){const trend=trendView(row.rankChange,row.previousRank);return `<span class="trend ${trend.className}" title="${trend.title}">${trend.label}</span>`;}
...
function podiumCard(row){return `<article class="rank-card podium-card ${nameLengthClass(row)} level-${row.level}" data-rank="${row.rank}" ${cardAttrs(row)}><span class="medal medal-${row.rank}" aria-label="第 ${row.rank} 名"><small>TOP</small><strong>${row.rank}</strong></span><div class="avatar avatar-large">${escapeHtml(avatarText(row.studentName))}</div><div class="student-name">${escapeHtml(row.studentName)}</div><div class="score-line"><span class="score">${row.totalPoints} 积分</span>${trendBadge(row)}</div></article>`;}
function nameTile(row){const band=row.rank<=10?"rank-4-10":row.rank<=30?"rank-11-30":"";return `<article class="rank-card name-tile ${band} ${nameLengthClass(row)} level-${row.level}" ${cardAttrs(row)}><span class="rank-badge">${row.rank}</span><div class="avatar">${escapeHtml(avatarText(row.studentName))}</div><div class="student-copy"><div class="student-name">${escapeHtml(row.studentName)}</div><div class="score-line"><span class="meta">${row.totalPoints} 积分</span>${trendBadge(row)}</div></div></article>`;}
...
function scoreDetails(row){return `<h3>${escapeHtml(row.studentName)} · ${row.totalPoints} 积分</h3><dl><div><dt>完课</dt><dd>${row.completionPoints}</dd></div><div><dt>课上作业</dt><dd>${row.inclassPoints}</dd></div><div><dt>课后作业</dt><dd>${row.homeworkPoints}</dd></div><div><dt>综合正确率</dt><dd>${Number(row.accuracyRate).toFixed(1)}%</dd></div></dl>`;}
