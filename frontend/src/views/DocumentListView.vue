...
const data = ref({ documents: [], total: 0, page: 0, pageCount: 0 });
const error = ref("");
...
async function load() {
  ...
  try { data.value = await api(`/documents?${params}`); }
  catch (failure) { error.value = failure.message; }
  finally { busy.value = false; }
}

function syncFilters() {
  filters.createdDate = route.query.createdDate || "";
  filters.updatedDate = route.query.updatedDate || "";
...
async function changeStatus(document) {
  ...
  try {
    await api(`/documents/${document.id}/status`, { method: "PATCH", body: jsonBody({ status: next }) });
    notify(next === "normal" ? "文档已上线" : "文档已下线");
    await load();
  } catch (failure) {
...
<template>
  <AdminLayout page-title="文档管理" active-page="documents">
    ...
    <form class="filter-bar" @submit.prevent="search">
      ...
      <div class="filter-actions"><button class="button button-quiet" type="submit">查询</button><RouterLink class="button button-quiet" to="/doc/list">重置</RouterLink><RouterLink v-if="hasPermission('documents.create')" class="button button-primary" to="/doc/create">新建</RouterLink></div>
    </form>
    <div class="document-table-wrap"><table class="document-table">
      ...
      <tbody>
        <tr v-for="document in data.documents" :key="document.id">
          <td><RouterLink v-if="hasPermission('documents.edit')" class="document-title-link" :to="`/doc/edit/${document.id}`">{{ document.title }}</RouterLink><strong v-else>{{ document.title }}</strong><span class="document-id">#{{ document.id }}</span></td>
          <td><span class="status-badge" :class="`status-${document.status}`">{{ statusLabel(document.status) }}</span></td><td>{{ formatDateTime(document.createdAt) }}</td><td>{{ formatDateTime(document.updatedAt) }}</td>
          <td><div class="row-actions"><button v-if="hasPermission('documents.share')" class="button button-quiet button-small" type="button" title="复制查看链接" @click="share(document)"><Share2 :size="14"/>分享</button><a class="button button-quiet button-small" :href="`/doc/show/${document.id}`">查看</a><RouterLink v-if="hasPermission('documents.edit')" class="button button-quiet button-small" :to="`/doc/edit/${document.id}`">编辑</RouterLink><button v-if="hasPermission('documents.status')" class="button button-small" :class="document.status === 'normal' ? 'button-danger' : 'button-primary'" type="button" @click="changeStatus(document)">{{ document.status === "normal" ? "下线" : "上线" }}</button></div></td>
        </tr>
